...
FROM
	UNDERWRITING.UNDERWRITINGCASE UC INNER JOIN UNDERWRITING.UNDERWRITINGCASERISK UR 
		ON UC.UNDERWRITINGCASEID = UR.UNDERWRITINGCASEID,
	(XMLTABLE('RiskInformation/Roles/Role/Client/ClientDocuments/ClientDocument' PASSING UR.RISKINFORMATIONDATA COLUMNS 
		...
		DOCUMENTNUMBER  VARCHAR2(80) PATH '/ClientDocument/@DocumentNUmber') DOC)
WHERE 
	REPLACE (REPLACE (REPLACE (DOCUMENTO.DOCUMENTNUMBER,'-',''),' ',''),'.','')= REPLACE (REPLACE (REPLACE ('25664715','-',''),' ',''),'.','')
